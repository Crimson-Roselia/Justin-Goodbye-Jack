...
public static class MathB
{
    ...
    /// <returns></returns>
    public static List<GameObject> ShapesOfGameObjects(GameObject pf, Vector3 centerPos, float radius, int amount, Shapes shape)
    {
        ...
        if (shape == Shapes.Wall)
        {
            for (int i = 0; i < amount; i++)
            {
                ...
                var pos = new Vector3(wallPos, 0, 1) + centerPos;
                //Instantiate
        ...
        else
        {
            ...
            for (int i = 0; i < amount; i++)
            {
                ...
                Vector3 position = new Vector3(Mathf.Cos(angle), 0, Mathf.Sin(angle))*radius + centerPos;
